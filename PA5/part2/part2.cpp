 ...
 * Title: CSCE451 PA5 part2.cpp
 * Date: 5/6/2022
 * Description: Virtual Memory Manager assuming limited memory space
 *
 ****************************************************************/
...
/***********************************************************
 * Function: handleLRU - handler for LRU parameter
 * Parameters: timer array
 * Return Value: position of replaced frame
 ***********************************************************/
int handleLRU(int timer[]);
		...
		while(fgets(address, 12, address_file) != NULL) {
			...
			if(frame_num == NOT_FOUND){
				...
				if(strcmp(argv[3], "lru")){
					if(free_frame >= FRAMES) {
						free_frame = 0;
					}
					for(int i = 0; i < TABLE_ENTRIES; i++){
						if(page_table[i] == free_frame){
							page_table[i] = -1;
						}
					}
